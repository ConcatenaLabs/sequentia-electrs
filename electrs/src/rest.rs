...
struct BlockValue {
    ...
    // SEQUENTIA: the Bitcoin (testnet4) block this Sequentia block is anchored
    // to. Parsed from the header by the rust-elements `sequentia` feature.
    #[cfg(feature = "sequentia")]
    #[serde(skip_serializing_if = "Option::is_none")]
    bitcoin_anchor: Option<BitcoinAnchorValue>,

    // SEQUENTIA: the decoded PoS BLS committee certificate (None if the block's
    // solution isn't a BLS cert, e.g. genesis or an escaping-stall block).
    #[cfg(feature = "sequentia")]
    #[serde(skip_serializing_if = "Option::is_none")]
...
// SEQUENTIA: a block's Bitcoin anchor (height + hash on Bitcoin testnet4).
#[cfg(feature = "sequentia")]
#[derive(Serialize, Deserialize)]
struct BitcoinAnchorValue {
    height: u32,
    hash: BlockHash,
}

// SEQUENTIA: one signing member of the PoS BLS committee, as carried in the
// block's proof solution (258 bytes: secp_pubkey(33) ‖ vrf_proof(81) ‖
// bls_pubkey(48) ‖ bls_pop(96); sizes from src/{pubkey,vrf,bls}.h).
#[cfg(feature = "sequentia")]
#[derive(Serialize, Deserialize)]
struct PosCommitteeMember {
    secp_pubkey: String,
    vrf_proof: String,
    bls_pubkey: String,
    bls_pop: String,
...
fn parse_pos_bls_certificate(solution: &Script) -> Option<PosCertificate> {
    use elements::script::Instruction::PushBytes;
    // The solution is a sequence of canonical data pushes; collect their payloads.
    let mut pushes: Vec<&[u8]> = Vec::new();
    for ins in solution.instructions() {
        match ins {
            Ok(PushBytes(b)) => pushes.push(b),
            // A non-push opcode (or malformed) means this isn't a BLS certificate
            // (e.g. an escaping-stall leader-only solution); don't force a shape.
            _ => return None,
        }
    }
    ...
    let members = pushes[2..]
        ...
        .map(|m| {
            ...
            Some(PosCommitteeMember {
                secp_pubkey: m[0..33].to_lower_hex_string(), // 33
                vrf_proof: m[33..114].to_lower_hex_string(), // 81
                bls_pubkey: m[114..162].to_lower_hex_string(), // 48
                bls_pop: m[162..258].to_lower_hex_string(), // 96
            })
        })
...
#[cfg(all(test, feature = "sequentia"))]
mod sequentia_cert_tests {
    use super::*;
    ...
    fn synth_member(seed: u8) -> Vec<u8> {
        ...
        m.extend(std::iter::repeat(seed ^ 0x01).take(81)); // vrf_proof
        m.extend(std::iter::repeat(seed ^ 0x02).take(48)); // bls_pubkey
        m.extend(std::iter::repeat(seed ^ 0x03).take(96)); // bls_pop
        m
    }

    #[test]
    fn parses_n_member_certificate() {
        for n in [1usize, 3, 51, 100] {
            let leader_sig = vec![0xABu8; 70];
            let agg_sig = vec![0xCDu8; 96];
            let members: Vec<Vec<u8>> = (0..n).map(|i| synth_member(i as u8)).collect();

            let mut b = Builder::new().push_slice(&leader_sig).push_slice(&agg_sig);
            for m in &members {
                b = b.push_slice(m);
            }
            let solution = b.into_script();
            ...
            for (i, m) in cert_members.iter().enumerate() {
                ...
                assert_eq!(m.bls_pubkey, raw[114..162].to_lower_hex_string(), "n={n} m={i} blspk");
    ...
    #[test]
    fn rejects_non_bls_solutions() {
        // leader-only (escaping-stall) solution: single push, no 96-byte aggregate.
        let leader_only = Builder::new().push_slice(&[0xAAu8; 70]).into_script();
        assert!(parse_pos_bls_certificate(&leader_only).is_none());
        // wrong aggregate length (e.g. a 64-byte MuSig2 aggregate).
        let musig = Builder::new()
            .push_slice(&[0xAAu8; 70])
            .push_slice(&[0xBBu8; 64])
            .into_script();
        assert!(parse_pos_bls_certificate(&musig).is_none());
    }
}
...
impl BlockValue {
    ...
    fn new(blockhm: BlockHeaderMeta) -> Self {
        ...
        BlockValue {
            ...
            #[cfg(feature = "sequentia")]
            bitcoin_anchor: header
                .bitcoin_anchor
                .map(|(height, hash)| BitcoinAnchorValue { height, hash }),

            #[cfg(feature = "sequentia")]
            pos_certificate: match &header.ext {
                elements::BlockExtData::Proof { solution, .. } => {
                    parse_pos_bls_certificate(solution)
                }
                _ => None,
            },

            // Filled in by the /block handler (needs an RPC call for finality).
            #[cfg(feature = "sequentia")]
            finalized: None,
        }
    }
...
fn handle_request(
    ...
    match (
        ...
        // SEQUENTIA: chain checkpoints / finality (getcheckpointinfo passthrough).
        #[cfg(feature = "sequentia")]
        ...
        (&Method::GET, Some(&"block"), Some(hash), None, None, None) => {
            ...
            let blockhm = query
                ...
                .ok_or_else(|| HttpError::not_found("Block not found".to_string()))?;
            // NOTE: /block is served purely from the index — no per-request
            // daemon RPC — so it stays fast and keeps working when the node
            // momentarily disconnects. Finality lives at /sequentia/checkpoints.
